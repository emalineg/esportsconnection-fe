...
const GuestForm = () => {
    ...
    const formSubmit = (e: FormEvent) => {
        ...
        submitGuestMutation.mutateAsync({
            guestName,
            guestContact,
            submitterName,
            submitterContact,
    ...
    return(
        <div className="text-indigo-900 w-full px-4">
            <div className="flex flex-col gap-4 bg-indigo-100 rounded-md justify-center items-center p-4">
                ...
                {submitted ? <>
                    <h2 className="text-center text-lg uppercase font-semibold">Submitted {guestName}</h2>
                    <button className="text-white font-semibold bg-indigo-800 flex flex-row p-2 rounded-md self-center uppercase col-span-1 md:col-span-2 justify-center" onClick={reset}>Submit Another</button>
                </> : <>
                    <form onSubmit={formSubmit} className="grid w-full md:w-1/2 columns-1 md:columns-2 gap-4">
                        ...
                        <button className="text-white font-semibold bg-indigo-800 flex flex-row p-2 rounded-md self-center uppercase col-span-1 md:col-span-2 justify-center" type="submit">Submit</button>
                    </form>
                </>}
            </div>
        </div>
